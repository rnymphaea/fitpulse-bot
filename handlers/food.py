...
from keyboards.common import start_keyboard, confirmation_keyboard, cancel_keyboard
from storage.food import get_product, create_product, get_products, create_meal
...
async def process_meal_name(message: Message, state: FSMContext):
    try:
        ...
        try:
            ...
            await state.set_data(data)
            await message.answer(f"Укажите, сколько вы съели блюда '{product.name}' в граммах")
        except IndexError:
